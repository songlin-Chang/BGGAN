...
def load_model_opt(netGTI, netGIT, netD, optim_G, optim_D, path, multi_gpus):
    checkpoint = torch.load(path, map_location=torch.device('cpu'))
    netGTI = load_model_weights(netGTI, checkpoint['model']['netGTI'], multi_gpus)
    netGIT = load_model_weights(netGIT, checkpoint['model']['netGIT'], multi_gpus)
    ...
    return netGTI, netGIT, netD, optim_G, optim_D


def load_models(netGTI, netGIT, netD, path):
    checkpoint = torch.load(path, map_location=torch.device('cpu'))
    netGTI = load_model_weights(netGTI, checkpoint['model']['netGTI'])
    netGTI = load_model_weights(netGIT, checkpoint['model']['netGIT'])
    netD = load_model_weights(netD, checkpoint['model']['netD'])
    return netGTI, netGIT, netD
...
def save_models(netGTI, netGIT, netD, optG, optD, epoch, multi_gpus, save_path):
    if (multi_gpus==True) and (get_rank() != 0):
        None
    else:
        state = {'model': {'netGTI': netGTI.state_dict(), 'netGIT': netGIT.state_dict(), 'netD': netD.state_dict()}, \
...
def get_GTI_in_out_chs(nf, imsize):
