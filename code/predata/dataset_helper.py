...
def combine_image_captions(images: List[str], captions_dict: Captions,
    ...
    for img in images:
        ...
        # select random 1 captions
        captions = captions_dict[img_id]
        idxs = np.random.choice(range(len(captions)), size=1, replace=False)
        captions_selected = [captions[i] for i in idxs]

        img_filename = images_dir + "/" + img["filename"]
...
def build_vocab(captions: List[chain],
    ...
    vocab: Vocab = Vocab(bag_of_words, min_freq=min_freq)
    return vocab
