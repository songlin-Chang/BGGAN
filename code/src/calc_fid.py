...
from lib.utils import load_netGTI, load_npz
...
def parse_args():
    # Training settings
    parser = argparse.ArgumentParser(description='BG-GAN')
...
def main(args):
    ...
    netGTI = load_netGTI(netGTI, model_path, multi_gpus, train=False)
    netGTI.eval()
    ...
    with torch.no_grad():
        fid = eval(valid_dl, text_encoder, netGTI, args.device, m1, s1, args.save_image, args.val_save_dir, \
                   args.sample_times, args.z_dim, args.batch_size, args.truncation, args.trunc_rate)
    end_t = time.time()
    if (multi_gpus == True) and (get_rank() != 0):
        None
    else:
        print('Sampling done, %.2fs cost, The FID is : %.2f and std is : %.2f' % (end_t - start_t, fid, np.std(fid)))
